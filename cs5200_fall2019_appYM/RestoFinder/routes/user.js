...
router.post('/register', (req, res) => {
  ...
  //console.log("recieve register request!");
    ...
    if (user) {
      ...
      if (req.body.password == req.body.confirmpassword) {
        ...
        if (req.body.userType == "CUSTOMER") {
          ...
          let follows = [];
          let followedBy = [];
          customer.endorse = endorse;
          customer.favourites = favourites;
          customer.follows = follows;
          customer.followedBy = followedBy;
...
router.post('/:id1/endorse/:id2', async (req, res) => {
  try {
    ...
    if(user1.userType == 'CUSTOMER') {
      ...
      return res.status(400).json({alreadyFollows: 'User has already endorsed the user'});
...
router.post('/:id1/unendorse/:id2', async (req, res) => {
  try {
    ...
    if (user1.userType == "CUSTOMER") {
      ...
      if (user1.owner.endorses.filter(user => user.equals(user2._id)).length == 0)
      return res.status(400).json({doesNotEndorse: 'CUSTOMER does not endorse this user'});
...
router.post('/:id1/follow/:id2', async (req, res) => {
  ...
  try {
    ...
    if (user1.userType == 'CUSTOMER') {
      ...
      console.log(user1.customer.follows[0]);
      console.log(user2._id);

      if (user1.customer.follows.filter(user => user.equals(user2._id)).length > 0)
      return res.status(400).json({alreadyFollows: 'User has already followed the user'});


      if (user2.userType == 'CUSTOMER' || user2.userType == 'OWNER') {

        user1.customer.follows.push(user2);

        user1.save();

        // registerd user is the other user
        if (user2.userType == 'CUSTOMER') {
          user2.customer.followedBy.push(user1);
          user2.save();

        }
        // critic is the second user
        else {
          user2.owner.followedBy.push(user1);
          user2.save();
        }
        //
        //
...
router.post('/:id1/unfollow/:id2', async (req, res) => {
  try {
    ...
    if (user1.userType == 'CUSTOMER') {
      ...
      // if(user1.registeredUser.follows.filter())
      console.log("user1 follows list", user1.customer.follows);
      ...
      if (user2.userType == 'CUSTOMER' || user2.userType == 'OWNER') {
        let removalindex = 0;
        user1.customer.follows.map((item, index) => {
          if(item.toString() == user2._id.toString())
          removalindex = index;
        });
        ...
        user1.customer.follows.splice(removalindex,1);

        user1.save();

        // customer user is the other user
        if (user2.userType == 'CUSTOMER') {

          user2.customer.followedBy.map((item, index) => {
            if(item.toString() == user1._id.toString())
            removalindex = index;
          });

          user2.customer.followedBy.splice(removalindex,1);

          user2.save();
        }
        else {

          user2.owner.followedBy.map((item, index) => {
            if(item.toString() == user1._id.toString())
            removalindex = index;
          });
          user2.owner.followedBy.splice(removalindex,1);
          user2.save();
        }
        //
        //
...
router.get('/:id/follows', async (req, res) => {
  try {
    ...
    else if(user.userType == 'CUSTOMER' || user.userType == 'ADMIN') {
      console.log('registered');
      let follows = user.customer.follows;
      console.log(follows);
...
router.get('/:id/followedby', async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).send("User not found");
      ...
      console.log('customer User type')
      let followedBy = user.customer.followedBy;
...
router.post('/:id1/favourites/:id2', async (req, res) => {
  try {
    ...
    if(user.userType == "CUSTOMER" || user.userType == "ADMIN") {
      ...
      if (user.customer.favourites.filter(rest => rest.equals(restId._id)).length > 0){
        console.log("start to push: " + user);
        return res.status(400).json({alreadyFavorite: 'User has already favourite the restaurant'});
      }
      else {

        user.customer.favourites.push(restId);
        //console.log("start to push: " + user);
        user.save();
        return res.json({user: user});
      }
...
// POST request
router.post('/:id1/unfavourites/:id2', async(req, res) => {
  try {
    const userid = req.params.id1;
    const restid = req.params.id2;
    ...
    if(user.userType == "CUSTOMER" || user.userType == "ADMIN") {
      ...
      let removalindex = 0;
      user.customer.favourites.map((item, index) => {
      ...
      user.customer.favourites.splice(removalindex,1);
...
router.get('/:id/favorites', async(req, res) => {
  try {
    ...
    if(user.userType == 'CUSTOMER' || user.userType == 'ADMIN') {
      let favorites = user.customer.favourites;
